...
Use for MotionValues, imperative `animate` / `useAnimate`, scroll linking, layout/`layoutId`, Reorder, and related hooks.
...
Returns `scrollX`/`scrollY` and `*Progress` (0–1). Prefer linking to `opacity` / `transform` / `filter` / `clipPath` for GPU path. Target measurement uses layout box (**ignores CSS transforms**).
...
- `layoutDependency`, `layoutScroll` (overflow scroll), `layoutRoot` (`position: fixed` ancestors), `layoutAnchor`.
...
Layout built-in. Drag handle via `useDragControls` + `dragListener={false}`. Works with `AnimatePresence`. Prefer `position: relative|absolute` so drag `z-index` works.
